...
async function details(req, res, next) {
  ...
  const {user} = req;
  try {
    const cube = await models.cubeModel.findById(id).populate('accessories').populate('creatorId');
    if (!cube) { res.redirect('/not-found'); return; }
    if (cube.creatorId.username === user.username) {
...
function postEdit(req, res) {
  const id = req.params.id;
  const { name = null, description = null, imageUrl = null, difficultyLevel = null } = req.body;
  models.cubeModel.updateOne({ _id: id }, { name, description, imageUrl, difficultyLevel: Number(difficultyLevel) + 1 }).then(cube => {
    res.redirect('/');
  });
}
